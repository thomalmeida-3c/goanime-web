...
- `GET /api/manga/home` — mangás populares (MangaLivre + MangaLivre.blog + MangaMillion, em paralelo). `GET /api/manga/latest` — mangás recém-adicionados (idem). `GET /api/manga/search?q=` — busca (idem). `GET /api/manga/chapters?id=<>&source=<>` — lista de capítulos (não se aplica a MangaMillion, que abre externamente). `GET /api/manga/pages?id=<>&source=<>` — resolve as URLs das páginas pro leitor. Ver "Mangás" abaixo.
...
## Mangás (MangaLivre + MangaLivre.blog + MangaMillion)

Home, "recém adicionados" e busca de mangá mostram **três fontes em paralelo**: `mangalivre.go` (`mangalivre.to`), `mangalivreblog.go` (`mangalivre.blog`) e `mangamillion.go` (o catálogo oficial da Shueisha). `fanMangaSources` (`manga_handlers.go`, variádica) dispara todas em paralelo e junta os resultados; uma fonte errar não derruba as outras.

**MangaLivre.to e MangaLivre.blog** — dois sites diferentes, sem relação entre si além do nome parecido (nem o mesmo tema: `.to` é Madara/WordPress, `.blog` é um tema próprio com outras classes CSS). Adicionados porque **cada um tem títulos que o outro não tem** — motivo real do segundo: o `.to` não tinha Vinland Saga, o `.blog` tinha. Scrapers de verdade (`goquery`), mas bem mais simples que qualquer fonte de anime: sem Cloudflare, sem sessão/token, `curl` puro já traz o HTML completo, capítulos numa página só (sem paginação), imagens estáticas no próprio domínio (sem proxy). Cada um usa a própria listagem de popularidade do site (`?m_orderby=views` / `?ordem=popular`) como ranking real.

**MangaMillion** (`mangamillion.shueisha.co.jp`) é diferente dos outros dois em três eixos: (1) é a **plataforma oficial da própria editora** (Shueisha), não um agregador de fã; (2) o catálogo (`/api/manga_list`, 394 títulos) é uma **API JSON... na verdade Protocol Buffers binário**, sem `.proto` publicado — decodificado à mão em Go (`encoding/protowire`, sem gerar código a partir de schema) depois de mapear os números de campo com `protoc --decode_raw` contra uma resposta real; (3) **só entra no catálogo, não no leitor**: ler um capítulo de verdade exige um `Access-Token` que o JS deles gera no cliente sem nenhuma chamada de rede visível (não é um cookie de sessão comum, e um token forjado no mesmo formato leva 403) — decidido não investir em descobrir o algoritmo, é a trava de acesso do próprio detentor dos direitos, categoria diferente de raspar um agregador não-oficial. Na prática: aparece na busca/fileiras com capa, título e autor reais vindos da API pública deles; ao abrir, o app manda pra `mangamillion.shueisha.co.jp/pt-BR/title/<id>` numa aba nova em vez de fingir que consegue ler por dentro (`page.tsx`, `handleSelectManga` checa `source === "MangaMillion"` antes de rotear pro nosso leitor).

**Adicionar uma quarta fonte** segue o molde do MangaLivre.blog: um arquivo novo (client HTTP + parser + as funções search/popular/latest/chapters/pages, um `Source` string próprio), somar no `fanMangaSources` dos 3 handlers de listagem, e um novo `case` nos `switch source` de `handleMangaChapters`/`handleChapterPages` — **a menos que, como o MangaMillion, a fonte não tenha capítulos abertos de verdade**, caso em que a UI deve linkar pra fora em vez de simular suporte. Sempre inspecionar a fonte de verdade primeiro (`curl`, `protoc --decode_raw` se vier binário, DevTools do navegador pra achar chamadas de API) — cada site tem sua própria estrutura, mesmo quando o nome sugere parentesco.
