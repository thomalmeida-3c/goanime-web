...
## Mangás (MangaDex + MangaLivre)

Duas fontes, cada uma com um ponto forte diferente — `source` (`"MangaDex"` ou `"MangaLivre"`) vem em todo `MangaItem` e precisa ser repassado pra `/api/manga/chapters` e `/api/manga/pages`, porque os dois esquemas de id não têm nada a ver entre si (UUID vs URL da página).

**MangaDex** (`mangadex.go`) — [API pública, gratuita, sem chave](https://api.mangadex.org), a mesma que a maioria dos leitores de mangá por aí usa. Sempre filtra `contentRating=safe,suggestive` e `availableTranslatedLanguage=pt-br`. A pegadinha: não dá a URL da imagem direto — primeiro pede um servidor de CDN em `/at-home/server/{chapterId}` (esquema deles de distribuir carga, "MangaDex@Home"), resolvido na hora que o capítulo é aberto, não antes. Onde ela apanha: títulos **licenciados oficialmente** (Boruto, Jujutsu Kaisen, ...) costumam não ter nenhum capítulo de fato hospedado em pt-br — só stubs `externalUrl` apontando pro site oficial (que filtramos, já que não levam a nenhuma página real), deixando a obra sem nenhum capítulo pra ler.

**MangaLivre** (`mangalivre.go`, `mangalivre.to`) — foi aqui que entrou pra cobrir esse buraco. É um scraper de verdade (tema WordPress "Madara", `goquery`, mesmo padrão dos scrapers de anime), mas **bem mais simples** que qualquer fonte de anime: sem Cloudflare, sem sessão/token, `curl` puro já traz o HTML completo. A lista de capítulos vem inteira renderizada numa página só (sem paginação pra perseguir), e as imagens são arquivos estáticos no próprio domínio — sem precisar de proxy nem headers especiais, ao contrário do vídeo de anime. Onde ele ganha do MangaDex: cobertura completa pra título licenciado (Boruto: Two Blue Vortex tem os 35 capítulos reais, contra zero no MangaDex).

**Busca unificada**: `/api/search` (anime) e `/api/manga/search` (mangá, que por sua vez já fande MangaDex+MangaLivre em paralelo) rodam em paralelo a cada busca; o frontend guarda os resultados por tipo e um filtro (Todos/Animes/Mangás) só troca o que é exibido. Dentro de mangá, os resultados das duas fontes são reordenados por relevância ao termo buscado (`rankMangaByRelevance`) — sem isso, um spin-off do MangaDex podia aparecer acima da série principal só por ter rankeado melhor na busca deles.
